...
(function(root, factory) {
  ...
  function hyphenateWord(text, patterns, debug, hyphenChar) {
    ...
    levels[0] = levels[levels.length - 1] = 0;
    ...
    return {
      next: function() {
        ...
        while ((nextChar = text.charAt(nextCharIndex++))) {
          var charIsSpaceOrSpecial = /[^a-zàáèéìíòóùúäëïöüç·ñ'’ŀâêîôû]/.test(nextChar.toLocaleLowerCase());
  ...
  function start(text, patterns, cache, debug, hyphenChar) {
    ...
    while ((nextWord = readWord.next())) {
      var state =
        nextWord.length > 1 ? states.hyphenateWord : states.concatenate;
